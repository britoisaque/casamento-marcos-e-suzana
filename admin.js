...
async function loadGuests() {
  ...
  catch (error) { console.error(error); $("#guest-table").innerHTML = '<tr><td colspan="4" class="error">Não foi possível carregar convidados.</td></tr>'; }
}
function renderGuests() {
  const filter = normalize($("#guest-filter").value); const rows = guests.filter(g => !filter || normalize(g.name).includes(filter));
  ...
  $("#guest-table").innerHTML = rows.length ? rows.map(g => `<tr><td>${escapeHtml(g.name)}</td><td>${escapeHtml(g.category || "Individual")}<br><small>${escapeHtml(g.inviteId || "—")}</small></td><td><span class="status ${g.status || "pending"}">${statusLabel(g.status)}</span></td><td><div class="row-actions"><button data-edit-guest="${g.id}">Editar</button><button data-delete-guest="${g.id}">Excluir</button></div></td></tr>`).join("") : '<tr><td colspan="4" class="loading">Nenhum convidado encontrado.</td></tr>';
...
function addFamilyMember(member = {}) {
  ...
  row.innerHTML = `<label>Nome<input class="family-member-name" required maxlength="120" value="${escapeHtml(member.name || "")}" placeholder="Nome da pessoa" /></label><button class="remove-member" type="button">Remover</button>`;
...
function familyEntries() { return Array.from(document.querySelectorAll(".family-member-row")).map(row => ({ name: row.querySelector(".family-member-name").value.trim() })); }
...
function editGuest(id) {
  const g = guests.find(item => item.id === id); if (!g) return; $("#guest-id").value = id; $("#guest-name").value = g.name || ""; $("#guest-category").value = g.category || "Individual"; $("#guest-invite-id").value = g.inviteId || ""; $("#guest-status").value = g.status || "pending"; $("#family-member-list").innerHTML = ""; updateFamilyForm();
...
async function saveGuest(event) {
  event.preventDefault(); const id = $("#guest-id").value; const name = $("#guest-name").value.trim(), category = $("#guest-category").value; const inviteId = $("#guest-invite-id").value.trim() || (category === "Individual" ? undefined : slug(name)); const status = $("#guest-status").value; const button = event.currentTarget.querySelector("button[type=submit]"); button.disabled = true;
  try {
    if (category === "Família") {
      const members = [{ name }, ...familyEntries()]; if (members.some(member => !member.name)) throw new Error("Preencha o nome de todas as pessoas da família.");
      ...
      const batch = writeBatch(db); members.forEach((member, index) => { const data = { name: member.name, category, inviteId, searchName: normalize(member.name), status: index === 0 ? status : (existing[index]?.status || "pending"), updatedAt: serverTimestamp() }; if (existing[index]) batch.update(doc(db, "guests", existing[index].id), data); else batch.set(doc(collection(db, "guests")), { ...data, createdAt: serverTimestamp() }); }); existing.slice(members.length).forEach(member => batch.delete(doc(db, "guests", member.id))); await batch.commit();
      feedback($("#guest-form-feedback"), `${members.length} pessoa(s) salvas no convite familiar.`, "success");
    } else { const data = { name, category, status, searchName: normalize(name), ...(inviteId ? { inviteId } : {}), updatedAt: serverTimestamp() }; if (id) await updateDoc(doc(db, "guests", id), data); else await addDoc(collection(db, "guests"), { ...data, createdAt: serverTimestamp() }); feedback($("#guest-form-feedback"), id ? "Convidado atualizado." : "Convidado adicionado.", "success"); }
